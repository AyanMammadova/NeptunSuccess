import React from 'react'
import { FaFacebookF } from 'react-icons/fa'
import { ImInstagram } from "react-icons/im"
import { TfiYoutube } from "react-icons/tfi";
import { FaLinkedinIn } from "react-icons/fa";
import { MdKeyboardArrowRight } from "react-icons/md";


function Footer() {
  return (
    <div >
      <div className='bg-[#FF8300] h-[756px] flex justify-between py-[25px] pl-[25px]'>
        <div className='w-[75%]'>
          <div className='flex justify-between items-start'>
            <div className='text-white flex flex-col lg:flex-row justify-center items-center lg:items-start text-[11px] lg:gap-7'>
              <div className='mb-3'>
                <img className='w-[40px] h-[30px] hover:w-[45px] hover:h-[35px] transition-all duration-300' src="https://neptun.az/image/footer_img/delivery.svg" />
              </div>
              <div className='text-start md:text-center flex flex-wrap'>
                <p>Pulsuz çatdırılma <br />50 azn-dən yuxarı</p>
              </div>
            </div>
            <div className='text-white flex flex-col lg:flex-row justify-center items-center lg:items-start text-[11px] lg:gap-7'>
              <div className='mb-3'>
                <img className='w-[35px] h-[30px] hover:w-[40px] hover:h-[35px] transition-all duration-300' src="https://neptun.az/image/footer_img/cash.svg" />
              </div>
              <div className='text-start md:text-center'>
                <p>Nağd və ya <br /> kartla ödəniş</p>
              </div>
            </div>
            <div className='text-white flex flex-col lg:flex-row justify-center items-center lg:items-start text-[11px] lg:gap-7'>
              <div className='mb-3'>
                <img className='w-[35px] h-[30px] hover:w-[40px] hover:h-[35px] transition-all duration-300' src="https://neptun.az/image/footer_img/gift.svg" />
              </div>
              <div className='text-start md:text-center'>
                <p>Hədiyyə <br /> kuponları</p>
              </div>
            </div>
            <div className='text-white flex-col lg:flex-row justify-center items-center lg:items-start text-[11px] lg:gap-7 hidden md:flex'>
              <div className='mb-3'>
                <img className='w-[35px] h-[30px] hover:w-[40px] hover:h-[35px] transition-all duration-300' src="https://neptun.az/image/footer_img/online.svg" />
              </div>
              <div className='text-start md:text-center'>
                <p>Onlayn <br /> müştəri xidmətləri</p>
              </div>
            </div>
            <div className='text-white flex-col lg:flex-row justify-center items-center lg:items-start text-[11px] lg:gap-7 hidden md:flex'>
              <div className='mb-3'>
                <img className='w-[25px] h-[30px] hover:w-[30px] hover:h-[35px] transition-all duration-300' src="https://neptun.az/image/footer_img/mobile.svg" />
              </div>
              <div className='text-start md:text-center'>
                <p>Sürətli <br /> mobil mağazası</p>
              </div>
            </div>
          </div>
          <div className='mt-[50px]'>
            <div>
              <div className='flex flex-col md:flex-row gap-4 md:gap-20 items-start md:items-center'>
                <div>
                  <p className='text-white font-bold text-[11px] '>Bizə abunə olun</p>
                </div>
                <div className='relative w-full max-w-[356px]'>
                  <input className='w-full h-[42px] px-5 text-[12px] border rounded-[20px]' type="email" placeholder="e-poçt ünvanınız..."  size="55" />
                  <button className="absolute h-[40px] right-[1px] top-1/2 transform -translate-y-1/2 bg-[#FF8300] hover:bg-[#E17400] transition-all duration-300 text-white text-[14px] px-5 rounded-[20px]">Abunə ol</button>
                </div>
              </div>
            </div>
            <div className='mt-[25px]'>
              <div className='flex flex-col md:flex-row gap-4 md:gap-11 items-start md:items-center'>
                <div>
                  <p className='text-white font-bold text-[11px] '>Sms xəbərdarlıq üçün</p>
                </div>
                <div className='w-full max-w-[356px] flex justify-between gap-2 md:gap-4'>
                  <div>
                    <select className='w-[70px] h-[40px] text-gray-400 text-[14px] px-2 rounded-[20px]'>
                      <option>---</option>
                      <option>050</option>
                      <option>051</option>
                      <option>055</option>
                      <option>070</option>
                      <option>077</option>
                    </select>
                  </div>
                  <div className='relative w-full'>
                    <input className='w-full h-[42px] px-5 text-[12px] border rounded-[20px]' type="text" placeholder="XXX YY ZZ"  size="55" />
                    <button className="absolute h-[40px] right-[1px] top-1/2 transform -translate-y-1/2 bg-[#FF8300] hover:bg-[#E17400] transition-all duration-300 text-white text-[14px] px-5 rounded-[20px]">Abunə ol</button>
                  </div>
                </div>
              </div>
            </div>
            <div className='mt-[25px]'>
              <div className='flex flex-col md:flex-row gap-4 md:gap-28 items-start md:items-center'>
                <div>
                  <p className='text-white font-bold text-[11px] '>Bizi izləyin</p>
                </div>
                <div>
                  <ul className='flex gap-2'>
                    <li>
                      <a className='block border border-white hover:border-[#FF8300] rounded-full text-[#FF8300] hover:text-white bg-white hover:bg-[#E17400] transition-all duration-300 p-2' href='#'><FaFacebookF className=' text-[24px]' /></a>
                    </li>
                    <li>
                      <a className='block border border-white hover:border-[#FF8300] rounded-full text-[#FF8300] hover:text-white bg-white hover:bg-[#E17400] transition-all duration-300 p-2' href='#'><ImInstagram className=' text-[24px]' /></a>
                    </li>
                    <li>
                      <a className='block border border-white hover:border-[#FF8300] rounded-full text-[#FF8300] hover:text-white bg-white hover:bg-[#E17400] transition-all duration-300 p-2' href='#'><TfiYoutube className=' text-[24px]' /></a>
                    </li>
                    <li>
                      <a className='block border border-white hover:border-[#FF8300] rounded-full text-[#FF8300] hover:text-white bg-white hover:bg-[#E17400] transition-all duration-300 p-2' href='#'><FaLinkedinIn className=' text-[24px]' /></a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
            <div className='mt-[25px]'>
              <div className='block md:hidden'>
                <p className='text-white text-[11px] '>Daha çox</p>
              </div>
              <div className='hidden md:flex gap-5 text-white text-[13px]'>
                <div>
                  <ul>
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Haqqımızda</a></li>
                    <li className='flex items-center py-[10px]'><MdKeyboardArrowRight /><a href="#">Siyasətimiz</a></li>
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Yeniliklər</a></li>
                  </ul>
                </div>
                <div>
                  <ul >
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Aksiyalar</a></li>
                    <li className='flex items-center pt-[10px]'><MdKeyboardArrowRight /><a href="#">Kampaniyalar</a></li>
                    <li className='flex items-center py-[10px]'><MdKeyboardArrowRight /><a href="#">Neptun bonus kart</a></li>
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Elektron kataloq</a></li>
                  </ul>
                </div>
                <div>
                  <ul>
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Mağazalarımız</a></li>
                    <li className='flex items-center py-[10px]'><MdKeyboardArrowRight /><a href="#">İrad və təkliflər</a></li>
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Alıcıların nəzərinə</a></li>
                    <li className='flex items-center py-[10px]'><MdKeyboardArrowRight /><a href="#">Partnyorluq</a></li>
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Supermarketdə reklam</a></li>
                  </ul>
                </div>
                <div>
                  <ul>
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Karyera</a></li>
                    <li className='flex items-center py-[10px]'><MdKeyboardArrowRight /><a href="#">İşə qəbul proseduru</a></li>
                    <li className='flex items-center'><MdKeyboardArrowRight /><a href="#">Vakansiyalar</a></li>
                    <li className='flex items-center pt-[10px]'><MdKeyboardArrowRight /><a href="#">CV yerləşdirin</a></li>
                  </ul>
                </div>
              </div>
            </div>
            <div className='mt-[80px]'>
              <div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  xmlnsXlink="http://www.w3.org/1999/xlink"
                  width="230px"
                  height="50px"
                  viewBox="0 0 2533 542"
                >
                  <image
                    id="Слой_1"
                    data-name="Слой 1"
                    width={2533}
                    height={542}
                    xlinkHref="data:img/png;base64,"
                  />
                </svg>
              </div>
            </div>
            <div className='mt-[70px]'>
              <div>
                <p className='text-white text-[13px]'>&copy; 2003 - 2023 Neptun Supermarket. All rights reserved</p>
              </div>
              <div className='flex items-center mt-[15px]'>
                <span className='text-white text-[13px] inline-block mr-2'>Created by:</span>
                <img className='w-[70px] h-[25px]' src="https://neptun.az/image/catalog/footer/amiroff_creative_agency.png" />
              </div>
            </div>
          </div>

        </div>
        <div className="relative w-[25%] overflow-hidden z-50">
          <div className="absolute lg:top-[14%] right-[-237px] md:right-[-168px] inset-0 z-10">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="442.03"
              height={565}
              viewBox="0 0 442.03 565"
            >
              <defs>
                <style
                  dangerouslySetInnerHTML={{
                    __html: `
              .cls-1 {
                fill: #fff;
                fill-rule: evenodd;
              }
            `
                  }}
                />
              </defs>
              <path
                id="symbol"
                className="cls-1"
                d="M1220.94,3035.86a168.127,168.127,0,0,0-101.87,34.66c-25.9-22.53-59.57-35.53-98.41-35.53h-11.23A60.72,60.72,0,0,0,949,3095.65c0,32.93,26.763,60.66,60.429,60.66h11.23c21.58,0,31.94,18.19,31.94,37.26v172.44a60.907,60.907,0,0,0,61.29,60.66c33.67,0,60.44-27.73,60.44-60.66V3206.57c0-26,21.58-47.66,47.48-47.66s47.48,21.66,47.48,47.66v159.44c0,32.93,26.76,60.66,60.43,60.66a60.916,60.916,0,0,0,61.3-60.66V3206.57C1390.15,3111.25,1314.18,3035.86,1220.94,3035.86Zm-107.91,442.81a60.655,60.655,0,1,1-60.43,60.66A60.536,60.536,0,0,1,1113.03,3478.67Zm215.83,0a60.655,60.655,0,1,1-60.43,60.66A60.543,60.543,0,0,1,1328.86,3478.67Z"
                transform="translate(-949 -3035)"
              />
            </svg>
          </div>
        </div>

      </div>
      <div className='bg-white flex justify-center py-[20px]'>
        <img className='w-[160px] h-[55px]' src="https://neptun.az/image/catalog/footer/visalogo.svg" />
      </div>
    </div>
  )
}

export default Footer